// Generates two seed files from the same deterministic data (seeded PRNG), so
// the output only changes when this script or its inputs change:
//   supabase/seed.sql          local stack: 300 synthetic Nashville cars, demo
//                              dealers, app_config defaults, demo accounts (buyer,
//                              dealer, admin, private seller) and a synthetic
//                              buyer panel for demand insights
//   supabase/seed-staging.sql  hosted projects: reference data and the same
//                              fictional dealers and cars, with no accounts
//
//   npm run fixtures
...
// Hosted staging/demo projects: the inventory, labeled demo inspection shops, one
// promoted car and feed rows. No accounts, passwords, swipes or leads.
function stagingPhase23Sql(): string {
  let out = "-- Demo inspection shops, a promoted car and dealer feed rows.\n";
  out += insert("public.inspection_shops", INSPECTION_SHOPS.map((sh, i) => ({
    id: uuidFrom(i + 1, "5b"), name: sh.name, address: sh.address, city: sh.city, zip: sh.zip, lat: sh.lat, lng: sh.lng,
    phone: `615-555-${String(300 + i).padStart(4, "0")}`, email: `inspections+${i + 1}@example.com`, price_usd: sh.price, rating: sh.rating, mobile: sh.mobile, is_demo: true,
  })));
  const promoted = listings.filter((g) => g.dealerIndex === 0 && g.row.condition !== "new")[3];
  out += `update public.listings set promoted_until = now() + interval '7 days' where id = '${promoted.row.id}';\n`;
  out += insert("public.promotions", [{ listing_id: promoted.row.id as string, dealership_id: demoDealerId, days: 7, amount_usd: DEFAULT_CONFIG.billing.promotion_price_usd, status: "dev", starts_at: raw("now()"), ends_at: raw("now() + interval '7 days'"), created_by: null }]);
  out += insert("public.dealer_feeds", dealerIds.map((id) => ({ dealership_id: id })));
  return out;
}
...
function main() {
  const reference = [
    insert("public.zip_codes", ZIPS.map((z) => ({ zip: z.zip, city: z.city, state: "TN", lat: z.lat, lng: z.lng }))),
    insert("public.markets", [{ id: "nashville", name: "Nashville metro", center_zip: "37203", radius_mi: 60, is_active: true, priority: 1 }]),
    configSql(DEFAULT_CONFIG),
  ];
  const inventory = [
    insert("public.dealerships", DEALERS.map((d, i) => ({
    ...
    insert("public.dealership_private", DEALERS.map((d, i) => ({
    ...
    insert("public.listings", [...listings.map((g) => g.row), ...duplicates]),
    insert("public.listing_photos", listings.flatMap((g) => g.photos.map((url, position) => ({ listing_id: g.row.id as string, url, position })))),
    insert("public.listing_embeddings", listings.map((g) => ({
      listing_id: g.row.id as string, model: "fixture-style-v1", embedding: raw(`'[${g.embedding.join(",")}]'::extensions.vector`),
    }))),
    insert("public.listing_price_changes", listings.filter((g) => g.priceDrop).map((g) => ({
  ...
  const ingestRun = `insert into public.ingest_runs (market_id, source, status, stats, finished_at)
...
`);
  parts.push(...reference, ...inventory);
  parts.push(demoUsersSql());
  parts.push(demoBuyerSql());
  parts.push(phase23Sql());
  parts.push(ingestRun);
  writeFileSync("supabase/seed.sql", parts.join("\n"));

  const staging: string[] = [];
  staging.push(`-- Generated by scripts/fixtures/generate.ts. Do not edit by hand; run \`npm run fixtures\`.
-- For hosted staging/demo projects, after the migrations. Load it once, in one transaction:
--   psql "$SUPABASE_DB_URL" -1 -v ON_ERROR_STOP=1 -f supabase/seed-staging.sql
--
-- Reference data (ZIPs, the Nashville market, app_config defaults) plus the same
-- 300 synthetic cars and fictional dealers as seed.sql (source 'fixture',
-- example.com websites, demo inspection shops). No accounts, passwords, swipes
-- or leads. Remove the fictional inventory before launch (see README).
`);
  staging.push(...reference, ...inventory);
  staging.push(stagingPhase23Sql());
  staging.push(ingestRun);
  writeFileSync("supabase/seed-staging.sql", staging.join("\n"));

  const counts = listings.reduce<Record<string, number>>((acc, g) => {
    acc[g.row.body_style as string] = (acc[g.row.body_style as string] ?? 0) + 1;
    return acc;
  }, {});
  console.log(`Wrote supabase/seed.sql and supabase/seed-staging.sql: ${listings.length} listings (+${duplicates.length} duplicate VINs)`, counts);
}
