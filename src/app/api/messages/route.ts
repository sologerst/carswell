...
export const POST = route(async (req: Request) => {
  ...
  // RLS: this select only succeeds for the buyer or the dealer's members.
  const { data: conv } = await supabase.from("conversations").select("id, buyer_id, dealership_id").eq("id", conversationId).maybeSingle();
  if (!conv) throw new ApiError(404, "Conversation not found.");
  const role = conv.buyer_id === profile.id ? "buyer" : "dealer";
  ...
  if (role === "buyer") {
    ...
    await admin.from("notifications").insert({
      user_id: conv.buyer_id, kind: "message", title: "New message from the dealer", body: body.slice(0, 140), url: `/chat/${conversationId}`,
    });
  }
